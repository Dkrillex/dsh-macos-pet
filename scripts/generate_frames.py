...
SKIN_SUBJECT = {
    ...
    "claude": (
        "Claude mascot creature (a soft round body in warm terracotta orange #D97757, "
        "a cream #F0EEE6 face plate, and a crest of tapered starburst rays fanning up and "
        "out from behind the head like a mane; two small dark friendly eyes, a tiny mouth, "
        "and two short rounded arms)"
    ),
...
# The rest pose for a skin that has no art yet, drawn from words alone because
# there is nothing to edit from. Everything the pose deltas later rely on has to
# be established here: the palette, the plate, the framing, and the body parts
# they name (`one hand raised beside the head`, `holding a single pencil`).
SEED_PROMPTS: dict[str, str] = {
    "claude": (
        "A cute chibi mascot character, centred, facing the viewer, drawn in a clean flat "
        "vector style with soft cel shading and a consistent medium line weight. The body is "
        "a soft rounded blob in warm terracotta orange #D97757 with a large cream #F0EEE6 face "
        "plate. Behind and above the head, a crest of eight tapered starburst rays in the same "
        "terracotta orange fans upward and outward like a mane, longest at the top. Two small "
        "dark #3D2B24 oval eyes, wide apart, and a tiny closed smile. Two short rounded arms at "
        "its sides. Calm, friendly, at rest. Full body in frame with generous even margins on "
        "all four sides, and the lowest point of the body well above the bottom edge. "
        "The background is pure saturated magenta #FF00FF, completely flat and uniform, filling "
        "the entire frame edge to edge behind the character, with no gradient, no shadow, no "
        "glow, no vignette and no ground plane. The character itself contains no magenta and no "
        "pink anywhere. No text, no letters, no words, no logo, no watermark."
    ),
}
...
def generate(
    ...
    if seed:
        # The one request in this script with no reference image. Refused rather
        # than silently redrawn when art already exists: overwriting a skin's
        # rest pose re-rolls the character, and every other frame is an edit of
        # this file, so the whole skin would drift out from under itself.
        if dest.is_file() and not force:
            return f"{skin}/{state}/{index}", "skip (exists)"
        prompt = SEED_PROMPTS.get(skin, "")
        if not prompt:
            return f"{skin}/{state}/{index}", f"FAIL no seed prompt for {skin}"
        reference = None
    elif replate:
        ...
        if dest.is_file() and not force:
            return f"{skin}/{state}/{index}", "skip (exists)"
        found = reference_for(skin, state, index)
        if not found.is_file():
            return f"{skin}/{state}/{index}", f"FAIL no reference at {found}"
        reference = found
    ...
    if BACKEND == "gemini":
        args, temps = _gemini_request(
            prompt, reference if reference is not None and reference.is_file() else None)
        try:
        ...
        return (f"{skin}/{state}/{index}",
                "FAIL seeding needs the gemini backend (/images/edits has no text-only mode)")
    else:
        url, key = _endpoint()
...
def main() -> int:
    ...
    parser.add_argument(
        "--seed",
        action="store_true",
        help="draw the missing idle/00 rest pose for a new skin, from words alone",
    )
    parser.add_argument(
        "--replate",
        action="store_true",
    ...
    if args.seed and not args.skin:
        # The default list is discovered from directories under assets/source,
        # and a skin being seeded has none yet — so an unqualified --seed would
        # silently do nothing at all.
        parser.error("--seed needs --skin: there is no directory to discover it from yet")

    skins = args.skin or sorted(p.name for p in SOURCE_ROOT.iterdir() if p.is_dir())

    if args.seed:
        jobs = [(skin, "idle", "00", "") for skin in skins]
    elif args.replate:
    ...
    for wave in waves:
        ...
        with ThreadPoolExecutor(max_workers=args.workers) as pool:
            futures = {
                pool.submit(generate, *job, force=args.force, replate=args.replate,
                            seed=args.seed): job
                for job in wave
            }
