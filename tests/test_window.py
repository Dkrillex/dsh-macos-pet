...
from dsh_macos_pet import app as app_mod, nswindow, packs
from dsh_macos_pet.app import DeskPetApp
from dsh_macos_pet.mapper import AgentActivity
from dsh_macos_pet.runtime import PetRuntime
from dsh_macos_pet.skins import list_skins


def _app(clock_ref):
    app = DeskPetApp(PetRuntime(), clock=lambda: clock_ref[0])
    # Keep the suite out of the real ~/.dsh-macos-pet; both files are covered
...
class MenuActionTests(unittest.TestCase):
    ...
    def test_the_github_entry_opens_the_repository(self) -> None:
        ...
        self.assertTrue(cmd[1].startswith("https://github.com/Dkrillex/dsh-macos-pet"),
