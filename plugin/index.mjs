...
export const name = 'dsh-macos-pet'
export const inject = ['webServer']

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..')
const LAUNCHER = path.join(ROOT, 'bin', 'dsh-macos-pet')
...
function registerSkillRoot(ctx) {
  ...
  for (const register of [ctx.skills?.addRoot, ctx.skills?.register, ctx.addSkillRoot]) {
    ...
    try {
      ...
      ctx.logger?.warn?.(`[dsh-macos-pet] could not register the skill directory: ${err.message}`)
      return () => {}
    }
  }
  ctx.logger?.info?.(
    `[dsh-macos-pet] this host exposes no skill-root hook; to use the skin skill, link ${SKILLS_DIR} into a DSH skills directory`)
  return () => {}
}
...
export function apply(ctx) {
  ...
  child.on('error', (err) => {
    ctx.logger?.warn?.(`[dsh-macos-pet] desktop companion did not start: ${err.message}`)
  })
  child.on('exit', (code, signal) => {
    ...
    ctx.logger?.warn?.(`[dsh-macos-pet] desktop companion exited: ${why}`)
  })
